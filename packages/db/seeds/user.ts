...
function createFakePosts(users: User[], total_amount_posts: number) {
    ...
    for (const user of users) {
        const amount_posts = faker.number.int({ min: 0, max: total_amount_posts > 10 ? 10 : total_amount_posts });
...
function createFakeFriends(users: User[], total_amount_friends: number) {
    const _friends = [];
    for (const user of users) {
        const amount_friends = faker.number.int({ min: 0, max: total_amount_friends > 5 ? 5 : total_amount_friends });
