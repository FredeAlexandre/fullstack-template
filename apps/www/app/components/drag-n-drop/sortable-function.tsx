...
interface SortableFunctionProps {
  id: string;
  name: string;
  content: string;
...
export function SortableFunction({ id, content }: SortableFunctionProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    <div
      ...
      className="my-4 flex h-fit w-80 flex-col rounded-lg border border-black bg-white font-medium text-black text-sm"
      {...attributes}
    >
